...
#include <mico/slam/cvsba/compiler.h>
#include <mico/slam/cvsba/sba.h>
#include <mico/slam/cvsba/sba_chkjac.h>
...
)
{
	...
	switch(stop){
	case 0:
	    printf("Stop criteria no 0: no stop criteria reached");
	    break;
	case 1:
	    printf("Stop criteria no 1: converged");
	    break;
	case 2:
	    printf("Stop criteria no 2: relative change in p is small, stop\n");
	    break;
	case 3:
	    printf("Stop criteria no 3: reached max iterations\n");
	    break;
	case 4:
	    printf("Stop criteria no 4: passed test equivalent to the relative reduction of the RMS reprojection error: sqrt(p_eL2)-sqrt(pdp_eL2)<eps4*sqrt(p_eL2) \n");
	    break;
	case 5:
	    printf("Stop criteria no 5: error is small, forced termination of outer loop\n");
	    break;
	case 6:
	    printf("Stop criteria no 6: too many failed attempts to increase the damping factor in sba_str_levmar_x()! Singular Hessian matrix?\n");
	    break;	
	case 7:
	    printf("Stop criteria no 7: initial p_eL2 is not finite\n");
	    break;
	default:
	    printf("Non defined stopping criteria.\n");
	    break;
	}
	fflush(stdout);
    }
